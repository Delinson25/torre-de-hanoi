...
// CREACION DE TODAS LAS VARIABLES TIPO STRUCT CON CAJA
...
int a[ARRAY_SIZE];
int arrayced;
  n = sizeof(c[i].cedu) / sizeof(*c[i].cedu);
...
// CICLO FOR PARA PEDIR LOS DATOS DE LAS DIFERENTES PERSONAS
  ...
  while (change == 1)
    {
      ...
      for(i=0; i <= 5; i++)
        {
          if(c[i].cedu > c[i+1].cedu)
          {
            c[i].cedu->cedula = arrayced;
            c[i].cedu->cedula = (c[i+1].cedu->cedula);
            c[i+1].cedu->cedula = arrayced;
            change = 1;
          }
        }
    }

  // ARRAY PARA ORGANIZAR LAS CEDULAS
for(i = 0; i < 5 ; i++)
  {
 ...
 printf("\n");
  printf("\n");
    ...
    // CICLO FOR PARA IMPRIMIR TODOS LOS DATOS DE CEDULA
      }
